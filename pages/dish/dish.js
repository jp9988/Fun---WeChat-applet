...
Page({
  ...
  getToken:function(base64){
    ...
    wx.request({
      ...
      success:res=>{
        that.dishId(res.data.access_token,base64)
      }
    })
  },

  //菜品识别
  dishId:function(token,base64){
    that = this;
      ...
      success:res=>{
        ...
        if(res.data.result != null){
          for(var i = 0;i<res.data.result.length;i++){
            var probability = res.data.result[i].probability
            content[i].name = res.data.result[i].name;
            content[i].score = probability.substring(0,probability.indexOf(".")+4);
            content[i].calorie = res.data.result[i].calorie;
          }
        }
        wx.hideLoading();
        wx.showToast({
